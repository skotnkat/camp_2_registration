...
def corrupt_noise(tensor, std=0.2):
    """Add Gaussian noise simulating low-dose CT"""
    noise = torch.randn_like(tensor) * std
    corrupted = tensor + noise
    corrupted = corrupted.clamp(0, 1)
    return corrupted
...
for pair in ["0001", "0002", "0003"]:
    ...
    plt.suptitle(f'Pair {pair} — Corrupted (Gaussian Noise)')
    plt.savefig(f"results/noise_corrupted_pair{pair}.png", dpi=150, bbox_inches='tight')
    plt.close()
