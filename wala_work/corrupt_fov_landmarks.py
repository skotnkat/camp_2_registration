...
def compute_tre_after(fixed_lm, moving_lm, phi_field, orig_shape):
...
for pair in ["0001", "0002", "0003"]:
    ...
    tre_after = compute_tre_after(fixed_lm, moving_lm, phi_field, orig_shape)
    ...
    plt.suptitle(f'Pair {pair} — Corrupted (FOV Middle Truncation)')
    plt.savefig(f"results/fov_mid_corrupted_pair{pair}.png", dpi=150, bbox_inches='tight')
    plt.close()
